...
# =========================
# EXECUTIVE SCORE BAR + KPI COMPLIANCE
# (tempel setelah st.success(spotlight))
# =========================
st.markdown("### 🏁 Executive Score")

# Target KPI SLA (hari) — untuk Direksi
kpi_days = st.number_input("Target KPI SLA (hari)", min_value=0.0, value=2.0, step=0.5, key="ana_kpi_days")
kpi_sec = kpi_days * 86400.0

# KPI Compliance: % transaksi yang SLA <= target
...
# Scorecards (ringkas, dashboard style)
s1, s2, s3, s4 = st.columns(4)

# 1) Volume growth
s1.metric(
    "Growth Volume",
    "-" if np.isnan(p_total) else f"{p_total:+.1f}%",
    delta=f"{d_total:+,} trx"
)

# 2) SLA change
if has_sla and np.isfinite(d_mean):
    # SLA makin kecil = makin bagus → beri label "membaik/memburuk"
    sla_dir = "membaik" if d_mean < 0 else ("memburuk" if d_mean > 0 else "stabil")
    s2.metric("Perubahan SLA", sla_dir, delta=seconds_to_sla_format(d_mean))
else:
...
if has_sla and np.isfinite(complianceA) and np.isfinite(complianceB):
    s3.metric(
        f"KPI Compliance ≤ {kpi_days:g} hari",
        f"{complianceB:.1f}%",
        delta=f"{d_comp:+.1f} poin"
    )
else:
    s3.metric(f"KPI Compliance ≤ {kpi_days:g} hari", "-", delta="-")

# 4) Coverage SLA (kualitas data)
...
# Highlight “Direksi-friendly” (1 paragraf)
exec_lines = []
exec_lines.append(f"• Volume {('naik' if d_total>0 else 'turun' if d_total<0 else 'stabil')} (Δ {d_total:+,} transaksi).")
if has_sla and np.isfinite(d_mean):
    exec_lines.append(f"• SLA {('membaik' if d_mean<0 else 'memburuk' if d_mean>0 else 'stabil')} (Δ {seconds_to_sla_format(d_mean)}).")
if has_sla and np.isfinite(d_comp):
    exec_lines.append(f"• KPI compliance ≤ {kpi_days:g} hari berubah {d_comp:+.1f} poin (B: {complianceB:.1f}%).")

st.info("\n".join(exec_lines))

# =========================
# WATERFALL DRIVER (Δ trx) — terkesan "wah"
# Tempel setelah EXEC SCORE (atau setelah grafik tren)
# =========================
...
if dim_candidates:
    dim_wf = st.selectbox("Driver berdasarkan", dim_candidates, key="ana_wf_dim")
    top_n_wf = st.slider("Top driver ditampilkan", 5, 30, 12, 1, key="ana_wf_topn")

    # hitung trx per dim untuk A & B
    ...
    # ambil top absolute driver agar dramatis
    w = w.sort_values("delta", key=lambda s: s.abs(), ascending=False).head(top_n_wf)

    # siapkan data waterfall
    w_plot = w[[dim_wf, "delta"]].copy()
    w_plot.rename(columns={dim_wf: "driver"}, inplace=True)
    w_plot["measure"] = "relative"

    # total A & total B untuk “bridge”
    bridge = pd.DataFrame([
        {"driver": f"Total ({labelA})", "delta": totalA, "measure": "absolute"},
        *w_plot.to_dict("records"),
        {"driver": f"Total ({labelB})", "delta": totalB, "measure": "absolute"},
    ])
    ...
    st.info("Waterfall driver butuh kolom NAMA VENDOR atau JENIS TRANSAKSI.")

# =========================
# TOP BOTTLENECK (Δ SLA) — ringkas untuk Direksi
# Tempel setelah Waterfall
# =========================
...
if has_sla and dim_candidates2:
    dim_bb = st.selectbox("Bottleneck berdasarkan", dim_candidates2, key="ana_bb_dim")
    top_n_bb = st.slider("Top bottleneck", 5, 30, 10, 1, key="ana_bb_topn")

    # agregasi SLA mean per dim di A & B (hari)
    def agg_sla(df, dim_col):
    ...
    ga = agg_sla(dfA, dim_bb).rename(columns={"trx": "trx_A", "sla_hari": "sla_A(hari)"})
    gb = agg_sla(dfB, dim_bb).rename(columns={"trx": "trx_B", "sla_hari": "sla_B(hari)"})
    bb = pd.merge(ga, gb, on=dim_bb, how="outer").fillna(0)
    bb["Δ_sla(hari)"] = bb["sla_B(hari)"] - bb["sla_A(hari)"]
    bb["Δ_trx"] = bb["trx_B"] - bb["trx_A"]

    # bottleneck = Δ_sla terbesar (positif) dengan volume cukup
    bb_show = bb.sort_values("Δ_sla(hari)", ascending=False).head(top_n_bb)
    st.dataframe(bb_show, use_container_width=True)
else:
    st.info("Bottleneck butuh kolom SLA + (NAMA VENDOR atau JENIS TRANSAKSI).")
