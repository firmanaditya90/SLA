...
lottie_rocket = load_lottieurl("https://assets3.lottiefiles.com/packages/lf20_jtkhrafb.json")
...
if uploaded_file is not None and is_admin:
    ...
    with st.spinner("🚀 Sedang memuat data..."):
        st_lottie(lottie_rocket, height=250, key="rocket-load")
        time.sleep(1.5)
    ...
    with st.spinner("🚀 Membaca data..."):
        st_lottie(lottie_rocket, height=250, key="rocket-load2")
        time.sleep(1.5)
        df_raw = pd.read_excel(DATA_PATH, header=[0, 1])
