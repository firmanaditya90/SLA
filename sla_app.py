...
# ==============================
# ============================================
# 📄 PDF REPORT (E-BOOK STYLE, FPDF2 VERSION)
# ============================================
...
class PDFReport(FPDF):
    def header(self):
        # Header tiap halaman (kecuali cover)
        if self.page_no() > 1:
            self.set_font("Helvetica", "B", 12)
            self.cell(0, 10, "SLA Payment Analyzer Report", align="C", ln=1)
            self.ln(2)
...
def build_pdf_fpdf(df_filtered, periode_col, selected_periode, available_sla_cols, proses_grafik_cols):
    ...
    pdf.cell(0, 80, "", ln=1)  # spacer
    pdf.cell(0, 10, "Laporan SLA Payment Analyzer", ln=1, align="C")
    pdf.set_font("Helvetica", "", 14)
    periode_text = f"Periode: {selected_periode[0]} s.d. {selected_periode[-1]}" if selected_periode else "-"
    pdf.cell(0, 10, periode_text, ln=1, align="C")
    pdf.ln(20)
    pdf.set_font("Helvetica", "I", 12)
    pdf.cell(0, 10, f"Diterbitkan: {datetime.now().strftime('%d %B %Y')}", ln=1, align="C")

    # ----------------- TOC -----------------
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 16)
    pdf.cell(0, 10, "Daftar Isi", ln=1)
    ...
    pdf.cell(0, 10, "Bab 1. Ringkasan Eksekutif", ln=1)
    pdf.set_font("Helvetica", "", 12)
    if "KEUANGAN" in df_filtered.columns and len(df_filtered) > 0:
        avg_sec = float(df_filtered["KEUANGAN"].mean())
        pdf.multi_cell(0, 8, f"Rata-rata SLA Keuangan adalah {seconds_to_sla_format(avg_sec)} "
                             f"(~{avg_sec/86400:.2f} hari).")
    if "TOTAL WAKTU" in available_sla_cols:
        avg_tot = float(df_filtered["TOTAL WAKTU"].mean())
        pdf.multi_cell(0, 8, f"Rata-rata TOTAL WAKTU adalah {avg_tot/86400:.2f} hari.")
    ...
    if "KEUANGAN" in df_filtered.columns:
        ...
        buf = save_fig_to_buffer(fig); plt.close(fig)
        pdf.image(buf, w=170)

    # ----------------- BAB 2 -----------------
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 14)
    pdf.cell(0, 10, "Bab 2. KPI SLA", ln=1)
    pdf.set_font("Helvetica", "", 12)
    saved_kpi = load_kpi()
    if "KEUANGAN" in df_filtered.columns:
        avg_sec = float(df_filtered["KEUANGAN"].mean())
        ...
        pdf.multi_cell(0, 8, f"Target KPI: {saved_kpi if saved_kpi else '-'} hari.\n"
                             f"Pencapaian rata-rata: {avg_days:.2f} hari.\nStatus: {status}")

    # (Lanjutkan pola ini untuk BAB 3, 4, 5, 6 dengan tabel ringkas & chart sesuai tab dashboard)
    # ----------------- BAB 6 -----------------
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 14)
    pdf.cell(0, 10, "Bab 6. Kesimpulan & Rekomendasi", ln=1)
    pdf.set_font("Helvetica", "", 12)
    pdf.multi_cell(0, 8,
        "Secara umum, metrik SLA menunjukkan performa yang konsisten. "
        "Fokus perbaikan dapat diarahkan ke proses dengan rata-rata hari tertinggi, "
...
# 🔽 STREAMLIT Tab PDF
# ==============================
with tab_report:  # atau tab_pdf sesuai penamaanmu
    st.subheader("📥 Download PDF (E-Book Report)")
    if st.button("🧾 Generate PDF Report"):
        try:
            pdf_bytes = build_pdf_fpdf(
