...
def _plot_to_rlimage(fig, w_cm=10, h_cm=8, dpi=150):
    buf = io.BytesIO()
...
def _two_col_centered(left, right, left_w=12, right_w=8):
...
def _first_page(canvas, doc):
    pw, ph = landscape(A4)
    try:
        canvas.drawImage(_img_reader(LOGO_ASDP_URL), pw/2 - 3*cm, ph - 15*cm,
