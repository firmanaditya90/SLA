...
def generate_poster_with_quote(quote, output_file="Poster_SLA_A4.png"):
    # ===== Buat background poster =====
    W, H = 1200, 1600  # ukuran poster A4
    bg = Image.new("RGB", (W, H), "lightblue")  # background warna biru muda

    # ===== Bubble coordinates =====
    bubble_coords = {"x": 100, "y": 100, "w": 800, "h": 400}
    ...
    bubble_draw.rectangle(
        [bubble_coords['x'], bubble_coords['y'],
         bubble_coords['x'] + bubble_coords['w'], bubble_coords['y'] + bubble_coords['h']],
    ...
    # Bubble tail
    tail = [
        (bubble_coords['x'] + bubble_coords['w'] - 80, bubble_coords['y'] + bubble_coords['h']),
        (bubble_coords['x'] + bubble_coords['w'] - 20, bubble_coords['y'] + bubble_coords['h']),
        (W - 220, H - 320)
    ]
    bubble_draw.polygon(tail, fill=(255, 255, 255, 230), outline="black")

    # Paste bubble layer ke background
    bg.paste(bubble_layer, (0, 0), bubble_layer)

    # ===== Render quote =====
    font_path = "arial.ttf"  # ganti sesuai font lokal
    font_size = 36
    font_quote = ImageFont.truetype(font_path, font_size)

    # Wrap text agar muat di bubble
    max_chars_per_line = int(bubble_coords['w'] / (font_size * 0.6))
    wrapped_text = textwrap.fill(quote, width=max_chars_per_line)

    draw = ImageDraw.Draw(bg)
    draw.multiline_text(
        (bubble_coords['x'] + 20, bubble_coords['y'] + 20),
        wrapped_text,
        font=font_quote,
        fill="black",
        align="center",
    ...
    # ===== Save hasil poster =====
    bg.save(output_file)
    print(f"✅ Poster berhasil dibuat: {output_file}")
    return bg

# ===== Contoh isi quote =====
quote = """Jangan pernah menunggu motivasi datang. 
Mulailah bergerak, maka motivasi akan mengikuti. 
Setiap langkah kecil adalah awal dari pencapaian besar."""

# Generate poster
poster = generate_poster_with_quote(quote)
poster.show()
