...
with tab_jumlah:
    ...
    # ======================================================
    # Helper khusus tab jumlah
    # ======================================================
    def jumlah_fmt_int(x):
        try:
            return f"{int(x):,}".replace(",", ".")
        except Exception:
            return "-"

    def jumlah_fmt_pct(x):
        try:
            if pd.isna(x):
                return "Baseline"
    ...
    def jumlah_growth_insight(df_growth):
        if df_growth is None or df_growth.empty:
            return {
                "status": "NO DATA",
                "headline": "Belum ada data jumlah transaksi yang dapat dianalisis.",
                "findings": ["Silakan pastikan periode dan data transaksi sudah tersedia."],
                "risks": ["Insight belum dapat dibentuk karena data kosong."],
                "actions": ["Upload atau perluas rentang periode untuk melihat pola pertumbuhan."],
            }

        d = df_growth.copy()
        total_trx = int(d["Jumlah"].sum())
        period_count = len(d)
        latest = d.iloc[-1]
        first = d.iloc[0]
        growth_valid = d["Growth (%)"].dropna()

        if period_count == 1 or growth_valid.empty:
            return {
                "status": "BASELINE",
                "headline": (
                    f"Periode {latest[periode_col]} menjadi baseline analisis dengan "
                    f"{jumlah_fmt_int(latest['Jumlah'])} transaksi. Growth antar-periode belum dapat dihitung "
                    f"karena hanya ada satu periode yang ditampilkan."
                ),
                "findings": [
                    f"Total transaksi pada periode aktif adalah {jumlah_fmt_int(total_trx)} transaksi.",
                    "Growth akan muncul otomatis ketika rentang periode mencakup minimal dua periode.",
                    "Baseline ini dapat digunakan sebagai titik awal monitoring tren volume transaksi.",
                ],
                "risks": [
                    "Belum dapat disimpulkan arah kenaikan atau penurunan volume karena tidak ada periode pembanding.",
                ],
                "actions": [
                    "Perluas filter periode untuk membaca perubahan volume secara periodik.",
                    "Gunakan minimal dua periode agar sistem dapat menghitung growth otomatis.",
                ],
            }

        avg_growth = float(growth_valid.mean())
        latest_growth = latest["Growth (%)"]
        max_growth_row = d.loc[d["Growth (%)"].idxmax()]
        min_growth_row = d.loc[d["Growth (%)"].idxmin()]
        positive_count = int((growth_valid > 0).sum())
        negative_count = int((growth_valid < 0).sum())
        stable_count = int((growth_valid == 0).sum())
        net_growth = ((latest["Jumlah"] - first["Jumlah"]) / first["Jumlah"] * 100) if first["Jumlah"] != 0 else float("nan")

        if latest_growth > 0 and avg_growth > 0:
            status = "ACCELERATING"
            headline = (
                f"Volume transaksi periode terakhir meningkat {jumlah_fmt_pct(latest_growth)} menjadi "
                f"{jumlah_fmt_int(latest['Jumlah'])} transaksi. Secara rata-rata, growth periode aktif berada di "
                f"{jumlah_fmt_pct(avg_growth)}, sehingga tren jumlah transaksi cenderung ekspansif."
            )
        elif latest_growth < 0 and avg_growth < 0:
            status = "CONTRACTION"
            headline = (
                f"Volume transaksi periode terakhir turun {jumlah_fmt_pct(latest_growth)} menjadi "
                f"{jumlah_fmt_int(latest['Jumlah'])} transaksi. Rata-rata growth periode aktif juga negatif "
                f"({jumlah_fmt_pct(avg_growth)}), sehingga perlu monitoring penyebab penurunan volume."
            )
        elif latest_growth < 0:
            status = "WATCHLIST"
            headline = (
                f"Periode terakhir mengalami penurunan {jumlah_fmt_pct(latest_growth)}, meskipun rata-rata growth "
                f"periode aktif masih {jumlah_fmt_pct(avg_growth)}. Pola ini perlu dipantau agar tidak menjadi tren berulang."
            )
        elif latest_growth > 0:
            status = "RECOVERY"
            headline = (
                f"Periode terakhir menunjukkan pemulihan dengan growth {jumlah_fmt_pct(latest_growth)}. "
                f"Kenaikan ini perlu dikonfirmasi pada periode berikutnya agar menjadi tren yang konsisten."
            )
        else:
            status = "STABLE"
            headline = (
                f"Periode terakhir relatif stabil dengan growth {jumlah_fmt_pct(latest_growth)}. "
                f"Volume transaksi berada pada {jumlah_fmt_int(latest['Jumlah'])} transaksi."
            )

        findings = [
            f"Total transaksi pada rentang periode aktif mencapai {jumlah_fmt_int(total_trx)} transaksi dari {period_count} periode.",
            f"Growth tertinggi terjadi pada periode {max_growth_row[periode_col]} sebesar {jumlah_fmt_pct(max_growth_row['Growth (%)'])} dengan tambahan {jumlah_fmt_int(max_growth_row['Penambahan'])} transaksi.",
            f"Growth terendah terjadi pada periode {min_growth_row[periode_col]} sebesar {jumlah_fmt_pct(min_growth_row['Growth (%)'])} dengan perubahan {jumlah_fmt_int(min_growth_row['Penambahan'])} transaksi.",
            f"Net growth dari periode awal ke periode akhir adalah {jumlah_fmt_pct(net_growth)}.",
        ]

        risks = []
        if negative_count > 0:
            risks.append(f"Terdapat {negative_count} periode dengan penurunan jumlah transaksi dibanding periode sebelumnya.")
        if positive_count > 0:
            risks.append(f"Terdapat {positive_count} periode yang menunjukkan pertumbuhan positif.")
        if stable_count > 0:
            risks.append(f"Terdapat {stable_count} periode yang relatif stabil tanpa perubahan volume.")
        if abs(avg_growth) > 30:
            risks.append("Rata-rata growth cukup ekstrem, sehingga perlu dicek apakah terdapat anomali data atau perubahan proses bisnis.")
        if not risks:
            risks.append("Tidak terdapat sinyal volatilitas besar pada periode yang ditampilkan.")

        actions = [
            "Gunakan periode dengan growth tertinggi sebagai referensi untuk memahami faktor pendorong volume transaksi.",
            "Lakukan pendalaman pada periode yang mengalami decline untuk melihat penyebab penurunan, baik dari sisi data masuk, proses approval, maupun pola penagihan.",
            "Pantau konsistensi growth minimal dalam 2-3 periode berikutnya sebelum menetapkan target operasional baru.",
        ]
    ...
    def jumlah_html_list(items):
        if not items:
            return "<li>Belum ada insight yang dapat ditampilkan.</li>"
        return "".join([f"<li>{html.escape(str(x))}</li>" for x in items])

    # ======================================================
    # Data jumlah dan growth
    # ======================================================
    jumlah_base = (
        df_filtered
    ...
    jumlah_base = jumlah_base.sort_values(
        by=periode_col,
        key=lambda x: pd.Categorical(x, categories=selected_periode, ordered=True),
    ).reset_index(drop=True)

    jumlah_base["Periode Sebelumnya"] = jumlah_base[periode_col].shift(1)
    jumlah_base["Jumlah Sebelumnya"] = jumlah_base["Jumlah"].shift(1)
    jumlah_base["Penambahan"] = jumlah_base["Jumlah"] - jumlah_base["Jumlah Sebelumnya"]
    jumlah_base["Growth (%)"] = jumlah_base.apply(
        lambda row: ((row["Jumlah"] - row["Jumlah Sebelumnya"]) / row["Jumlah Sebelumnya"] * 100)
        if pd.notna(row["Jumlah Sebelumnya"]) and row["Jumlah Sebelumnya"] != 0
        else pd.NA,
        axis=1,
    )
    jumlah_base["Growth (%)"] = pd.to_numeric(jumlah_base["Growth (%)"], errors="coerce")
    jumlah_base["Penambahan"] = pd.to_numeric(jumlah_base["Penambahan"], errors="coerce")
    jumlah_base["Status"] = jumlah_base["Growth (%)"].apply(jumlah_growth_status)

    insight = jumlah_growth_insight(jumlah_base)

    total_transaksi_jumlah = int(jumlah_base["Jumlah"].sum()) if not jumlah_base.empty else 0
    jumlah_periode = len(jumlah_base)
    latest_row = jumlah_base.iloc[-1] if not jumlah_base.empty else None
    growth_valid = jumlah_base["Growth (%)"].dropna() if not jumlah_base.empty else pd.Series(dtype=float)
    avg_growth = float(growth_valid.mean()) if not growth_valid.empty else pd.NA
    latest_growth = latest_row["Growth (%)"] if latest_row is not None else pd.NA
    highest_growth = jumlah_base["Growth (%)"].max(skipna=True) if not jumlah_base.empty else pd.NA

    status_class_map = {
        "ACCELERATING": "jumlah-status-good",
        "RECOVERY": "jumlah-status-good",
        "WATCHLIST": "jumlah-status-watch",
        "CONTRACTION": "jumlah-status-critical",
        "STABLE": "jumlah-status-neutral",
        "BASELINE": "jumlah-status-neutral",
        "NO DATA": "jumlah-status-critical",
    }
    status_label = insight.get("status", "NO DATA")
    status_class = status_class_map.get(status_label, "jumlah-status-neutral")

    latest_period_text = "-" if latest_row is None else str(latest_row[periode_col])
    latest_jumlah_text = "-" if latest_row is None else jumlah_fmt_int(latest_row["Jumlah"])

    # ======================================================
    # AI Insight modern card
    # ======================================================
    ai_html = f"""
    <!DOCTYPE html>
    <html>
    ...
    .jumlah-ai-wrap {{
        ...
        color: #ffffff;
        background:
            radial-gradient(circle at 8% 8%, rgba(0,234,255,0.38), transparent 28%),
            radial-gradient(circle at 92% 18%, rgba(127,0,255,0.35), transparent 32%),
            radial-gradient(circle at 80% 88%, rgba(56,239,125,0.24), transparent 26%),
            linear-gradient(135deg, #041226 0%, #092a56 48%, #031a2d 100%);
        border: 1px solid rgba(255,255,255,0.18);
        box-shadow: 0 24px 70px rgba(0,0,0,0.38);
    }}
    .jumlah-ai-wrap:before {{
        content: "";
        position: absolute;
        inset: 0;
        background-image:
    ...
    .jumlah-status-good {{ background: rgba(56,239,125,0.20); color: #d9ffe9; }}
    .jumlah-status-watch {{ background: rgba(254,225,64,0.22); color: #fff4b0; }}
    .jumlah-status-critical {{ background: rgba(255,65,108,0.24); color: #ffdce5; }}
    .jumlah-status-neutral {{ background: rgba(255,255,255,0.14); color: #ffffff; }}
    .jumlah-headline {{
        position: relative;
    ...
    @media (max-width: 1000px) {{
        .jumlah-kpi-grid {{ grid-template-columns: repeat(2, 1fr); }}
        .jumlah-ai-grid {{ grid-template-columns: 1fr; }}
    }}
    </style>
    </head>
    <body>
    <div class="jumlah-ai-wrap">
        <div class="jumlah-ai-header">
            <div>
                <div class="jumlah-ai-title">AI Growth Intelligence</div>
                <div class="jumlah-ai-subtitle">
                    Analisis otomatis jumlah transaksi per periode berdasarkan filter periode aktif.
                </div>
            </div>
            <div class="jumlah-status {status_class}">{html.escape(status_label)}</div>
        </div>

        <div class="jumlah-headline">{html.escape(str(insight.get('headline', '-')))}</div>

        <div class="jumlah-kpi-grid">
            <div class="jumlah-kpi">
                <div class="jumlah-kpi-label">Total transaksi</div>
                <div class="jumlah-kpi-value">{jumlah_fmt_int(total_transaksi_jumlah)}</div>
                <div class="jumlah-kpi-sub">Akumulasi seluruh periode aktif</div>
            </div>
            <div class="jumlah-kpi">
                <div class="jumlah-kpi-label">Periode aktif</div>
                <div class="jumlah-kpi-value">{jumlah_fmt_int(jumlah_periode)}</div>
                <div class="jumlah-kpi-sub">Jumlah periode yang dibandingkan</div>
            </div>
            <div class="jumlah-kpi">
                <div class="jumlah-kpi-label">Periode terakhir</div>
                <div class="jumlah-kpi-value">{latest_jumlah_text}</div>
                <div class="jumlah-kpi-sub">Jumlah transaksi pada {html.escape(latest_period_text)}</div>
            </div>
            <div class="jumlah-kpi">
                <div class="jumlah-kpi-label">Growth terakhir</div>
                <div class="jumlah-kpi-value">{jumlah_fmt_pct(latest_growth)}</div>
                <div class="jumlah-kpi-sub">Dibanding periode sebelumnya</div>
            </div>
            <div class="jumlah-kpi">
                <div class="jumlah-kpi-label">Avg growth</div>
                <div class="jumlah-kpi-value">{jumlah_fmt_pct(avg_growth)}</div>
                <div class="jumlah-kpi-sub">Rata-rata growth periode aktif</div>
            </div>
        </div>

        <div class="jumlah-ai-grid">
            <div class="jumlah-box">
                <h4>Key Findings</h4>
                <ul>{jumlah_html_list(insight.get('findings', []))}</ul>
            </div>
            <div class="jumlah-box">
                <h4>Signal Detection</h4>
                <ul>{jumlah_html_list(insight.get('risks', []))}</ul>
            </div>
            <div class="jumlah-box">
                <h4>Recommended Actions</h4>
                <ul>{jumlah_html_list(insight.get('actions', []))}</ul>
            </div>
        </div>
        <div class="jumlah-ai-foot">Growth dihitung sebagai perubahan jumlah transaksi terhadap periode sebelumnya.</div>
    </div>
    </body>
    </html>
    """

    dynamic_height = 690 + 24 * max(
        len(insight.get("findings", [])),
        len(insight.get("risks", [])),
        len(insight.get("actions", [])),
    )
    components.html(ai_html, height=dynamic_height, scrolling=False)

    # ======================================================
    # Combo chart: jumlah transaksi dan growth
    # ======================================================
    st.markdown("### Growth Analytics per Periode")

    if jumlah_base.empty:
        st.info("Tidak ada data jumlah transaksi untuk periode yang dipilih.")
    else:
        fig_growth = make_subplots(specs=[[{"secondary_y": True}]])

        fig_growth.add_trace(
            go.Bar(
                x=jumlah_base[periode_col],
                y=jumlah_base["Jumlah"],
                name="Jumlah Transaksi",
                text=jumlah_base["Jumlah"].apply(jumlah_fmt_int),
                textposition="outside",
                marker=dict(
                    color=jumlah_base["Jumlah"],
                    colorscale="Blues",
                    line=dict(width=0),
                ),
                hovertemplate="Periode: %{x}<br>Jumlah: %{y:,.0f}<extra></extra>",
            ),
            secondary_y=False,
        )
        ...
        fig_growth.add_trace(
            go.Scatter(
                x=jumlah_base[periode_col],
                y=jumlah_base["Growth (%)"],
                name="Growth (%)",
                mode="lines+markers+text",
                text=jumlah_base["Growth (%)"].apply(jumlah_fmt_pct),
                textposition="top center",
                line=dict(width=4),
                marker=dict(size=12, line=dict(width=2, color="white")),
                hovertemplate="Periode: %{x}<br>Growth: %{y:.2f}%<extra></extra>",
            ),
            secondary_y=True,
        )

        fig_growth.update_layout(
            title="Jumlah Transaksi vs Growth per Periode",
            height=560,
            margin=dict(l=20, r=20, t=70, b=55),
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
            hovermode="x unified",
        )
        fig_growth.update_xaxes(title_text="Periode")
        fig_growth.update_yaxes(title_text="Jumlah Transaksi", secondary_y=False)
        fig_growth.update_yaxes(title_text="Growth (%)", secondary_y=True)
        st.plotly_chart(fig_growth, use_container_width=True)

    # ======================================================
    # Tabel jumlah + growth
    # ======================================================
    st.markdown("### Tabel Growth Jumlah Transaksi")

    jumlah_display = jumlah_base.copy()
    jumlah_display["Jumlah Sebelumnya"] = jumlah_display["Jumlah Sebelumnya"].fillna(0).astype(int)
    jumlah_display["Penambahan"] = jumlah_display["Penambahan"].fillna(0).astype(int)
    jumlah_display["Growth Display"] = jumlah_display["Growth (%)"].apply(jumlah_fmt_pct)
    ...
    jumlah_display = jumlah_display[display_cols_jumlah].rename(columns={"Growth Display": "Growth (%)"})

    total_row = pd.DataFrame({
        periode_col: ["TOTAL"],
        "Jumlah Sebelumnya": ["-"],
        "Jumlah": [jumlah_fmt_int(total_transaksi_jumlah)],
        "Penambahan": ["-"],
        "Growth (%)": [jumlah_fmt_pct(avg_growth)],
        "Status": ["AVG GROWTH"],
    })

    jumlah_table = pd.concat([jumlah_display, total_row], ignore_index=True)

    def style_jumlah_growth(row):
        status = str(row.get("Status", ""))
        if status == "GROWTH":
            return ["background-color: #dff8e8; color: #064e2e; font-weight: 700"] * len(row)
        if status == "DECLINE":
            return ["background-color: #ffe2e2; color: #7a0000; font-weight: 700"] * len(row)
        if status == "STABLE":
            return ["background-color: #e9f2ff; color: #06315f; font-weight: 700"] * len(row)
        if status == "BASELINE":
            return ["background-color: #f2f2f2; color: #333333; font-weight: 700"] * len(row)
        if status == "AVG GROWTH":
            return ["background-color: #102a58; color: white; font-weight: 900"] * len(row)
        return [""] * len(row)

    st.dataframe(
        jumlah_table.style.apply(style_jumlah_growth, axis=1),
        use_container_width=True,
        hide_index=True,
    )

    # ======================================================
    # Legacy simple chart tetap disediakan dalam expander
    # ======================================================
    with st.expander("Lihat grafik sederhana jumlah transaksi", expanded=False):
        fig_trans, ax_trans = plt.subplots(figsize=(10, 5))
        ax_trans.bar(
            jumlah_base[periode_col],
            jumlah_base["Jumlah"],
            color="#ff9f7f",
        )
