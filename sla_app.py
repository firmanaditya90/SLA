...
def build_auto_headline(totalA, totalB, meanA, meanB, p_total, d_total):
    ...
    vol_part = f"Volume {vol_dir} {_id_num(abs(p_total),2,'%')}" if (p_total is not None and not (isinstance(p_total,float) and np.isnan(p_total))) else f"Volume {vol_dir}"

    # SLA
    if meanA is None or meanB is None or (isinstance(meanA,float) and np.isnan(meanA)) or (isinstance(meanB,float) and np.isnan(meanB)):
        sla_part = "SLA: data belum lengkap"
    else:
        d = meanB - meanA
        sla_dir = "membaik" if d < 0 else ("memburuk" if d > 0 else "stabil")
        # % improvement = (A-B)/A * 100 (positif jika membaik)
        imp = None
        if meanA and meanA != 0:
            imp = (meanA - meanB) / meanA * 100
        if imp is None or (isinstance(imp,float) and np.isnan(imp)):
            sla_part = f"SLA {sla_dir}"
        else:
            sla_part = f"SLA {sla_dir} {_id_num(abs(imp),2,'%')}"
    return f"{sla_part} • {vol_part}"

def _load_logo_pil(logo_bytes):
    try:
...
def make_exec_poster_png(
    *,
    labelA, labelB,
    seriesA, seriesB,
    totalA, totalB, p_total, d_total,
    meanA, meanB, d_mean,
    kpi_days, complianceA, complianceB, d_comp,
    coverageA, coverageB, d_cov,
    vol_month_df=None,  # expects columns: Bulan, seriesA, seriesB
    sla_month_df=None,  # expects columns: Bulan, seriesA, seriesB (hari)
    headline="",
    logo_bytes=None,
    brand_title="EXECUTIVE SUMMARY",
    brand_subtitle="Analisis SLA & Transaksi",
):
    """
    Return PNG bytes (poster landscape A4-like).
    - Uses matplotlib + PIL (no external fonts).
    """
    import matplotlib.pyplot as plt
    from PIL import Image, ImageDraw, ImageFont

    # --- base canvas via matplotlib (easier layout) ---
    dpi = 200
    W, H = 1754, 1240  # ~A4 landscape @ 150-200dpi look
    fig = plt.figure(figsize=(W/dpi, H/dpi), dpi=dpi)
    fig.patch.set_facecolor("white")

    # Header area
    ax0 = fig.add_axes([0.03, 0.86, 0.94, 0.12])
    ax0.axis("off")
    ax0.text(0.00, 0.65, brand_title, fontsize=22, fontweight="bold")
    ax0.text(0.00, 0.20, brand_subtitle, fontsize=14, fontweight="bold", alpha=0.85)
    ax0.text(0.99, 0.65, datetime.now().strftime("%d %b %Y %H:%M"), ha="right", fontsize=10, alpha=0.7)
    ax0.text(0.99, 0.20, f"Periode: {labelA} vs {labelB}", ha="right", fontsize=11, fontweight="bold", alpha=0.85)

    # Logo (optional)
    if logo_bytes:
        logo = _load_logo_pil(logo_bytes)
        if logo is not None:
            # draw logo onto fig using imshow in a small axes
            ax_logo = fig.add_axes([0.86, 0.875, 0.11, 0.095])
            ax_logo.axis("off")
            ax_logo.imshow(logo)

    # KPI cards area
    def card(axpos, title, value, sub, badge=None, badge_is_good=True):
        ax = fig.add_axes(axpos)
        ax.axis("off")
        # draw rounded card using PIL later? We'll just emulate with rectangle
        ax.add_patch(plt.Rectangle((0, 0), 1, 1, fill=False, linewidth=1, alpha=0.15))
        ax.text(0.04, 0.80, title, fontsize=10, fontweight="bold", alpha=0.80)
        ax.text(0.04, 0.45, value, fontsize=22, fontweight="bold")
        ax.text(0.04, 0.16, sub, fontsize=9, alpha=0.75)
        if badge:
            # badge background
            color = (0.05, 0.55, 0.36, 0.15) if badge_is_good else (0.75, 0.10, 0.10, 0.15)
            edge  = (0.05, 0.55, 0.36, 0.30) if badge_is_good else (0.75, 0.10, 0.10, 0.30)
            ax.add_patch(plt.Rectangle((0.66, 0.65), 0.30, 0.20, facecolor=color, edgecolor=edge, linewidth=1))
            ax.text(0.81, 0.75, badge, ha="center", va="center", fontsize=9, fontweight="bold")

    # KPI values
    vol_badge = f"{_id_int(d_total)} ({_id_num(p_total,2,'%')})" if p_total is not None and not (isinstance(p_total,float) and np.isnan(p_total)) else _id_int(d_total)
    slaA_short = _sla_short_days(meanA, 2)
    slaB_short = _sla_short_days(meanB, 2)
    d_sla = (meanB - meanA) if (meanA is not None and meanB is not None and not (isinstance(meanA,float) and np.isnan(meanA)) and not (isinstance(meanB,float) and np.isnan(meanB))) else np.nan
    d_sla_short = _sla_short_days(abs(d_sla), 2) if not (isinstance(d_sla,float) and np.isnan(d_sla)) else "-"
    d_sla_short = (("-" if d_sla < 0 else "+") + d_sla_short) if d_sla_short != "-" else "-"

    comp_val = "-" if (complianceB is None or (isinstance(complianceB,float) and np.isnan(complianceB))) else _id_num(complianceB,2,"%")
    comp_badge = "-" if (d_comp is None or (isinstance(d_comp,float) and np.isnan(d_comp))) else _id_num(d_comp,2," poin")
    cov_val  = "-" if (coverageB is None or (isinstance(coverageB,float) and np.isnan(coverageB))) else _id_num(coverageB,2,"%")
    cov_badge = "-" if (d_cov is None or (isinstance(d_cov,float) and np.isnan(d_cov))) else _id_num(d_cov,2," poin")

    card([0.03, 0.70, 0.23, 0.13], f"Total Transaksi ({seriesA})", _id_int(totalA), "Baseline", None)
    card([0.27, 0.70, 0.23, 0.13], f"Total Transaksi ({seriesB})", _id_int(totalB), "Perubahan vs A", vol_badge, badge_is_good=(d_total>=0))
    card([0.51, 0.70, 0.23, 0.13], f"Avg SLA ({seriesA})", slaA_short, _sla_long_id(meanA), None)
    # SLA membaik jika delta negatif
    card([0.75, 0.70, 0.22, 0.13], f"Avg SLA ({seriesB})", slaB_short, _sla_long_id(meanB), d_sla_short, badge_is_good=(d_sla<0 if not (isinstance(d_sla,float) and np.isnan(d_sla)) else True))

    # Headline / highlight
    axH = fig.add_axes([0.03, 0.64, 0.94, 0.05]); axH.axis("off")
    axH.add_patch(plt.Rectangle((0, 0), 1, 1, facecolor=(0.12, 0.72, 0.35, 0.10), edgecolor=(0.12, 0.72, 0.35, 0.25), linewidth=1))
    axH.text(0.02, 0.50, headline if headline else "-", va="center", fontsize=12, fontweight="bold", alpha=0.9)

    # Executive Score (small row)
    axS = fig.add_axes([0.03, 0.56, 0.94, 0.07]); axS.axis("off")
    axS.text(0.00, 0.65, "Executive Score", fontsize=14, fontweight="bold")
    axS.text(0.00, 0.18, f"Target KPI SLA: {_id_num(kpi_days,2,' hari')}", fontsize=10, alpha=0.75)

    # score cards
    card([0.03, 0.41, 0.23, 0.13], "Growth Volume", _id_num(p_total,2,"%") if not (isinstance(p_total,float) and np.isnan(p_total)) else "-", "Δ transaksi vs A", _id_int(d_total)+" trx", badge_is_good=(d_total>=0))
    sla_word = "membaik" if (not (isinstance(d_sla,float) and np.isnan(d_sla)) and d_sla<0) else ("memburuk" if (not (isinstance(d_sla,float) and np.isnan(d_sla)) and d_sla>0) else "stabil")
    card([0.27, 0.41, 0.23, 0.13], "Perubahan SLA", sla_word, "Ringkas (hari)", d_sla_short, badge_is_good=(d_sla<0 if not (isinstance(d_sla,float) and np.isnan(d_sla)) else True))
    card([0.51, 0.41, 0.23, 0.13], f"KPI Compliance ≤ {_id_num(kpi_days,2,' hari')}", comp_val, "Δ poin vs A", comp_badge, badge_is_good=(d_comp>=0 if not (isinstance(d_comp,float) and np.isnan(d_comp)) else True))
    card([0.75, 0.41, 0.22, 0.13], "Coverage SLA", cov_val, "Kualitas data SLA", cov_badge, badge_is_good=(d_cov>=0 if not (isinstance(d_cov,float) and np.isnan(d_cov)) else True))

    # Trend charts (overlay)
    # Volume trend
    axV = fig.add_axes([0.05, 0.07, 0.44, 0.30])
    axV.set_title("Tren Volume (bulan)", fontsize=12, fontweight="bold", loc="left")
    if isinstance(vol_month_df, pd.DataFrame) and "Bulan" in vol_month_df.columns:
        x = list(range(len(vol_month_df["Bulan"])))
        axV.plot(x, vol_month_df[seriesA].values, marker="o", linewidth=2, label=str(seriesA))
        axV.plot(x, vol_month_df[seriesB].values, marker="o", linewidth=2, label=str(seriesB))
        axV.set_xticks(x)
        axV.set_xticklabels(vol_month_df["Bulan"].tolist(), rotation=0, fontsize=8)
    ...
    # SLA trend
    axT = fig.add_axes([0.53, 0.07, 0.44, 0.30])
    axT.set_title("Tren SLA (hari, bulan)", fontsize=12, fontweight="bold", loc="left")
    if isinstance(sla_month_df, pd.DataFrame) and "Bulan" in sla_month_df.columns:
        x = list(range(len(sla_month_df["Bulan"])))
        axT.plot(x, sla_month_df[seriesA].values, marker="o", linewidth=2, label=str(seriesA))
        axT.plot(x, sla_month_df[seriesB].values, marker="o", linewidth=2, label=str(seriesB))
        axT.set_xticks(x)
        axT.set_xticklabels(sla_month_df["Bulan"].tolist(), rotation=0, fontsize=8)
    ...
    # Footer
    axF = fig.add_axes([0.03, 0.01, 0.94, 0.04]); axF.axis("off")
    axF.text(0.00, 0.50, "Catatan: Coverage SLA = % transaksi yang memiliki nilai SLA valid (tidak kosong).", fontsize=9, alpha=0.70)
    axF.text(1.00, 0.50, "Sumber: Tab Analisis (hasil filter aktif)", fontsize=9, alpha=0.70, ha="right")
...
def make_exec_poster_pdf_from_png(png_bytes, filename_title="Executive Summary"):
    """
    Convert PNG poster -> 1-page PDF (A4 landscape).
    """
...
with tab_analisis:
    ...
    # -------------------------
    use_sidebar_filter = st.toggle("Gunakan filter sidebar (df_filtered)", value=True, key="ana_exec_usefilter_final2")
    ...
    def format_sla_id(seconds: float):
        """
        Return (short, long).
        short: '3,22 hari' (2 desimal)
        long : '3 hari 17 jam 41 menit'
        """
        if seconds is None or (isinstance(seconds, float) and np.isnan(seconds)):
            return "-", "-"
        try:
        ...
        days = int(sec // 86400)
        sec_rem = int(sec % 86400)
        hours = int(sec_rem // 3600)
        sec_rem = int(sec_rem % 3600)
        mins = int(sec_rem // 60)
        ...
        short_days = sec / 86400.0
        short_txt = f"{short_days:.2f} hari".replace(".", ",")

        if neg:
            short_txt = f"-{short_txt}"
            long_txt = f"-{long_txt}"

        return short_txt, long_txt
    ...
    # -------------------------
    # CSS lebih kontras + angka lebih "kelihatan"
    # -------------------------
    st.markdown("""
<style>
...
.kpi-title{font-size:13px;color:rgba(15,23,42,.78);margin:0 0 6px 0;font-weight:600}
.kpi-value{font-size:40px;font-weight:800;line-height:1.1;margin:0;color:rgb(15,23,42)}
.kpi-sub{font-size:12px;color:rgba(15,23,42,.70);margin-top:6px;font-weight:600}
.badge{
...
.small-cap{color:rgba(15,23,42,.72);font-weight:650}
</style>
""", unsafe_allow_html=True)
    ...
    with colS:
        ...
        if default_sla:
            sla_pick = st.selectbox("Pilih SLA", sla_options, index=sla_options.index(default_sla), key="ana_exec_sla_final2")
        else:
            sla_pick = None
    ...
    if mode == "By Tahun (kumulatif)":
        with colA:
            yearA = st.selectbox("Tahun A", years, index=0, key="ana_exec_yearA_final2")
        with colB:
            yearB = st.selectbox("Tahun B", years, index=min(1, len(years) - 1), key="ana_exec_yearB_final2")
        ...
        with colA:
            mA = st.selectbox("Bulan A", period_labels, index=0, key="ana_exec_monthA_final2")
        with colB:
            mB = st.selectbox("Bulan B", period_labels, index=min(1, len(period_labels) - 1), key="ana_exec_monthB_final2")
        ...
        with colA:
            sA = st.selectbox("Mulai A", period_labels, index=0, key="ana_exec_startA_final2")
            eA = st.selectbox("Sampai A", period_labels, index=len(period_labels) - 1, key="ana_exec_endA_final2")
        with colB:
            sB = st.selectbox("Mulai B", period_labels, index=0, key="ana_exec_startB_final2")
            eB = st.selectbox("Sampai B", period_labels, index=len(period_labels) - 1, key="ana_exec_endB_final2")
    ...
    if has_sla and np.isfinite(meanA) and np.isfinite(meanB):
        slaA_short, slaA_long = format_sla_id(meanA)
        slaB_short, slaB_long = format_sla_id(meanB)
        d_sla_sec = meanB - meanA
        sign = "-" if d_sla_sec < 0 else "+"
        abs_short, _ = format_sla_id(abs(d_sla_sec))
        d_sla_short = f"{sign}{abs_short}"

    badge_vol = f"{d_total:+,}".replace(",", ".") + ("" if np.isnan(p_total) else f" ({_fmt_pct(p_total)})")
    badge_sla = d_sla_short if (has_sla and np.isfinite(meanA) and np.isfinite(meanB)) else "-"

    html_cards = f"""
    <div class="kpi-wrap">
      <div class="kpi-card">
        <p class="kpi-title">Total Transaksi ({labelA})</p>
        <p class="kpi-value">{_fmt_int(totalA)}</p>
        <div class="kpi-sub">Baseline periode A</div>
      </div>

      <div class="kpi-card">
        <p class="kpi-title">Total Transaksi ({labelB})</p>
        <p class="kpi-value">{_fmt_int(totalB)}</p>
      ...
      <div class="kpi-card">
        ...
        <p class="kpi-value">{slaA_short}</p>
        <div class="kpi-sub">{slaA_long}</div>
      </div>

      <div class="kpi-card">
        <p class="kpi-title">Avg SLA ({labelB})</p>
        <p class="kpi-value">{slaB_short}</p>
        <span class="badge {badge_class(-1 if (has_sla and np.isfinite(meanA) and np.isfinite(meanB) and (meanB-meanA)<0) else (1 if (has_sla and np.isfinite(meanA) and np.isfinite(meanB) and (meanB-meanA)>0) else 0))}">{badge_sla}</span>
        <div class="kpi-sub">{slaB_long}</div>
      </div>
    </div>
    """
    ...
    vol_dir = "naik" if d_total > 0 else ("turun" if d_total < 0 else "stabil")
    spot = f"• Volume transaksi <b>{vol_dir}</b>: {_fmt_int(totalA)} → {_fmt_int(totalB)} (Δ {d_total:+,}".replace(",", ".") + ("" if np.isnan(p_total) else f", {_fmt_pct(p_total)}") + ")."
    if has_sla and np.isfinite(meanA) and np.isfinite(meanB):
        sla_dir = "membaik" if (meanB - meanA) < 0 else ("memburuk" if (meanB - meanA) > 0 else "stabil")
        spot += f" • SLA <b>{sla_dir}</b>: <b>{slaA_short}</b diketahui → <b>{slaB_short}</b> (Δ {badge_sla})."
    else:
        spot += " • SLA: <b>tidak tersedia</b>."
    st.markdown(f'<div class="hero">{spot}</div>', unsafe_allow_html=True)

    # =========================
    # (7) EXECUTIVE SCORE — CARD style + KPI ikut tab_overview (load_kpi)
    # =========================
    ...
    st.markdown(f'<div class="small-cap">Target KPI SLA (mengikuti Overview): <b>{kpi_days:.2f} hari</b></div>', unsafe_allow_html=True)

    complianceA = complianceB = d_comp = np.nan
    ...
    if has_sla and np.isfinite(d_mean):
        sla_dir_word = "membaik" if d_mean < 0 else ("memburuk" if d_mean > 0 else "stabil")

    badge_comp = "-" if (not has_sla or not np.isfinite(complianceA) or not np.isfinite(complianceB)) else f"{d_comp:+.2f} poin".replace(".", ",")
    badge_cov  = "-" if (not has_sla or not np.isfinite(coverageA) or not np.isfinite(coverageB)) else f"{(coverageB-coverageA):+.2f} poin".replace(".", ",")

    html_score = f"""
    <div class="kpi-wrap">
      <div class="kpi-card">
        <p class="kpi-title">Growth Volume</p>
        <p class="kpi-value">{("-" if np.isnan(p_total) else f"{p_total:+.2f}%".replace(".", ","))}</p>
        <span class="badge {badge_class(d_total)}">{f"{d_total:+,} trx".replace(",", ".")}</span>
        <div class="kpi-sub">Δ transaksi vs A</div>
      </div>

      <div class="kpi-card">
        <p class="kpi-title">Perubahan SLA</p>
        <p class="kpi-value">{sla_dir_word}</p>
        <span class="badge {badge_class(-1 if (has_sla and np.isfinite(d_mean) and d_mean<0) else (1 if (has_sla and np.isfinite(d_mean) and d_mean>0) else 0))}">{("-" if not (has_sla and np.isfinite(d_mean)) else badge_sla)}</span>
        <div class="kpi-sub">Ringkas (hari)</div>
      </div>

      <div class="kpi-card">
        <p class="kpi-title">KPI Compliance ≤ {kpi_days:.2f} hari</p>
        <p class="kpi-value">{("-" if (not has_sla or not np.isfinite(complianceB)) else f"{complianceB:.2f}%".replace(".", ","))}</p>
        <span class="badge {badge_class(0 if badge_comp=='-' else d_comp)}">{badge_comp}</span>
        <div class="kpi-sub">Δ poin vs A</div>
      </div>

      <div class="kpi-card">
        <p class="kpi-title">Coverage SLA</p>
        <p class="kpi-value">{("-" if (not has_sla or not np.isfinite(coverageB)) else f"{coverageB:.2f}%".replace(".", ","))}</p>
        <span class="badge {badge_class(0 if badge_cov=='-' else (coverageB-coverageA))}">{badge_cov}</span>
    ...
    # =========================
    # (8) TREN — X = bulan saja, warna = Tahun/Label A vs B
    # =========================
    ...
    if dim_candidates:
        dim_wf = st.selectbox("Driver berdasarkan", dim_candidates, key="ana_exec_wf_dim_final2")
        top_n_wf = st.slider("Top driver ditampilkan", 5, 30, 12, 1, key="ana_exec_wf_topn_final2")
        ...
        for _, r in w.iterrows():
            bridge_rows.append({"driver": str(r[dim_wf]), "delta": float(r["delta"]), "measure": "relative"})
        bridge_rows.append({"driver": f"Total ({labelB})", "delta": totalB, "measure": "absolute"})
        bridge = pd.DataFrame(bridge_rows)

        fig_wf = go.Figure(go.Waterfall(
            orientation="v",
            measure=bridge["measure"].tolist(),
            x=bridge["driver"].tolist(),
            y=bridge["delta"].tolist(),
            connector={"line": {"width": 1}}
        ))
        fig_wf.update_layout(
            title=f"Kontribusi {dim_wf} terhadap perubahan volume",
            xaxis_title=None,
        ...
        with st.expander("📋 Detail driver (tabel)"):
            st.dataframe(w.sort_values("delta", ascending=False), use_container_width=True)
    else:
    ...
    if has_sla and dim_candidates2:
        dim_bb = st.selectbox("Bottleneck berdasarkan", dim_candidates2, key="ana_exec_bb_dim_final2")
        top_n_bb = st.slider("Top bottleneck", 5, 30, 10, 1, key="ana_exec_bb_topn_final2")

        def agg_sla_dim(df, dim_col):
            x = df[[dim_col, sla_pick]].copy()
            x[sla_pick] = pd.to_numeric(x[sla_pick], errors="coerce")
            g = x.groupby(dim_col).agg(trx=(dim_col, "size"), sla_mean=(sla_pick, "mean")).reset_index()
            g["sla_hari"] = g["sla_mean"] / 86400.0
            return g[[dim_col, "trx", "sla_hari"]]

        ga = agg_sla_dim(dfA, dim_bb).rename(columns={"trx": "trx_A", "sla_hari": "sla_A(hari)"})
        gb = agg_sla_dim(dfB, dim_bb).rename(columns={"trx": "trx_B", "sla_hari": "sla_B(hari)"})

        bb = pd.merge(ga, gb, on=dim_bb, how="outer").fillna(0)
        bb["Δ_sla(hari)"] = bb["sla_B(hari)"] - bb["sla_A(hari)"]
        bb["Δ_trx"] = bb["trx_B"] - bb["trx_A"]

        st.caption("Tip: Δ_sla(hari) **positif** = makin lama (memburuk). Δ_sla(hari) **negatif** = makin cepat (membaik).")
        st.dataframe(bb.sort_values("Δ_sla(hari)", ascending=False).head(top_n_bb), use_container_width=True)
    else:
        st.info("Bottleneck butuh kolom SLA + (NAMA VENDOR/JENIS TRANSAKSI).")

    # =========================
    # (11) INSIGHT OTOMATIS — tabel ringkas (baris = bulan)
    # =========================
    ...
    exec_notes = []
    exec_notes.append(f"• Total transaksi {colA_name}: **{int(tbl[colA_name].sum()):,}** | {colB_name}: **{int(tbl[colB_name].sum()):,}**".replace(",", "."))
    if np.isfinite(p_total):
        exec_notes.append(f"• Growth total: **{p_total:+.2f}%** (Δ {d_total:+,} trx)".replace(".", ","))
    if has_sla and np.isfinite(d_mean):
        exec_notes.append(f"• Perubahan SLA rata-rata: **{badge_sla}**")
    if has_sla and np.isfinite(complianceB):
        exec_notes.append(f"• KPI compliance ≤ {kpi_days:.2f} hari: **{complianceB:.2f}%**".replace(".", ","))
    st.info("\n".join(exec_notes))
    ...
    with st.expander("📋 Detail perhitungan (opsional)"):
        ...
        st.write(f"**Periode B:** {labelB}")
        st.dataframe(dfA.head(50), use_container_width=True)
        st.dataframe(dfB.head(50), use_container_width=True)

# =========================
# POSTER EXECUTIVE SUMMARY (PNG + PDF) — berdasarkan filter & mode tab_analisis
# =========================
st.markdown("---")
st.markdown("### 🖼️ Poster Executive Summary (Direksi)")

colP1, colP2, colP3 = st.columns([1, 1, 1])
with colP1:
    brand_title = st.text_input("Judul Poster", value="EXECUTIVE SUMMARY", key="poster_title")
with colP2:
    brand_subtitle = st.text_input("Sub Judul", value="Analisis SLA & Transaksi", key="poster_subtitle")
with colP3:
    add_logo = st.toggle("Gunakan Logo (Corporate)", value=True, key="poster_use_logo")

logo_bytes = None
if add_logo:
    logo_file = st.file_uploader("Upload Logo (PNG/JPG)", type=["png", "jpg", "jpeg"], key="poster_logo")
    if logo_file is not None:
        logo_bytes = logo_file.read()

# Pastikan nama seri untuk poster (2024/2025)
seriesA = year_or_label(selA, "A")
seriesB = year_or_label(selB, "B")

# Build data tren bulan (yang sudah kamu pakai di chart tren)
months_union = sorted({int(p.month) for p in selA} | {int(p.month) for p in selB})
if not months_union:
    months_union = list(range(1, 13))
month_order = [month_id[m] for m in months_union]
...
if has_sla:
    ...
    A_sla = sla_by_monthnum(dfA)
    B_sla = sla_by_monthnum(dfB)
    sla_month_df = pd.DataFrame({
        "Bulan": month_order,
        seriesA: [float(A_sla.get(m, np.nan)) / 86400.0 for m in months_union],
        seriesB: [float(B_sla.get(m, np.nan)) / 86400.0 for m in months_union],
    })

# Headline otomatis (Direksi style)
headline = build_auto_headline(totalA, totalB, meanA, meanB, p_total, d_total)

# KPI target sama seperti Overview
saved_kpi = load_kpi() if "load_kpi" in globals() else None
kpi_days = float(saved_kpi) if saved_kpi is not None else 1.5

# Delta coverage
d_cov = (coverageB - coverageA) if (has_sla and np.isfinite(coverageA) and np.isfinite(coverageB)) else np.nan

# Generate poster bytes (cache agar cepat)
@st.cache_data(show_spinner=False)
def _gen_poster_cached(payload: dict):
    png = make_exec_poster_png(**payload)
    pdf = make_exec_poster_pdf_from_png(png, filename_title="Executive Summary Analisis")
    return png, pdf
...
payload = dict(
    ...
    meanA=meanA, meanB=meanB, d_mean=d_mean,
    kpi_days=kpi_days, complianceA=complianceA, complianceB=complianceB, d_comp=d_comp,
    coverageA=coverageA, coverageB=coverageB, d_cov=d_cov,
    vol_month_df=vol_month_df,
    sla_month_df=sla_month_df,
    headline=headline,
...
png_bytes, pdf_bytes = _gen_poster_cached(payload)

# Preview poster (wow factor)
st.image(png_bytes, caption="Preview Poster (siap WA / Direksi)", use_container_width=True)
...
with cD1:
    st.download_button(
        ...
        data=png_bytes,
        file_name="Poster_Executive_Summary.png",
        mime="image/png",
        use_container_width=True
    )
with cD2:
    st.download_button(
        ...
        data=pdf_bytes,
        file_name="Poster_Executive_Summary.pdf",
